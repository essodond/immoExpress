...
const BASE = 'C:\\Users\\DELL\\Documents\\projet\\immo';
...
function writeFile(filePath, content) {
  ...
  console.log('  FILE', filePath.replace(BASE, ''));
}
...
[
  backendBase,
  path.join(backendBase, 'src'),
  path.join(backendBase, 'src', 'utils'),
  path.join(backendBase, 'src', 'middleware'),
  path.join(backendBase, 'src', 'controllers'),
  path.join(backendBase, 'src', 'routes'),
  path.join(backendBase, 'prisma'),
].forEach(d => { mkdirp(d); console.log('  DIR ', d.replace(BASE, '')); });

// ── Backend files ───────────────────────────────────────────────────────────
writeFile(path.join(backendBase, 'package.json'), `{
  "name": "immo-backend",
  "version": "1.0.0",
...
writeFile(path.join(backendBase, '.env.example'), `DATABASE_URL="postgresql://user:password@localhost:5432/immo_db"
...
writeFile(path.join(backendBase, '.gitignore'), `node_modules/
.env
dist/
*.log
.DS_Store
...
writeFile(path.join(backendBase, 'prisma', 'schema.prisma'), `generator client {
...
writeFile(path.join(backendBase, 'prisma', 'seed.js'), `'use strict';
...
writeFile(path.join(backendBase, 'src', 'utils', 'cloudinary.js'), `'use strict';
...
writeFile(path.join(backendBase, 'src', 'utils', 'jwt.js'), `'use strict';
...
writeFile(path.join(backendBase, 'src', 'middleware', 'authMiddleware.js'), `'use strict';
...
writeFile(path.join(backendBase, 'src', 'middleware', 'validate.js'), `'use strict';
...
writeFile(path.join(backendBase, 'src', 'middleware', 'errorHandler.js'), `'use strict';
...
writeFile(path.join(backendBase, 'src', 'controllers', 'authController.js'), `'use strict';
...
writeFile(path.join(backendBase, 'src', 'controllers', 'maisonController.js'), `'use strict';
...
const update = async (req, res, next) => {
  try {
    ...
    const maison = await prisma.maison.update({ where: { id: req.params.id }, data: result.data });
    res.json({ success: true, data: maison });
  } catch (error) { next(error); }
};
...
writeFile(path.join(backendBase, 'src', 'controllers', 'uploadController.js'), `'use strict';
...
const fileFilter = (req, file, cb) => {
  ...
  else cb(new Error('Format de fichier non supporté. Utilisez JPEG, PNG ou WebP.'), false);
};
...
const uploadPhotos = [
  ...
  async (req, res, next) => {
    try {
      ...
      if (!maison) return res.status(404).json({ success: false, message: 'Maison introuvable.' });
      const hasMain = await prisma.photo.findFirst({ where: { maisonId: req.params.maisonId, isMain: true } });
      ...
      const photos = await Promise.all(
        uploadResults.map((result, index) =>
          prisma.photo.create({
            data: { url: result.url, publicId: result.publicId, isMain: !hasMain && index === 0, maisonId: req.params.maisonId },
          })
        )
      );
...
const deletePhoto = async (req, res, next) => {
  try {
    ...
    if (photo.isMain) {
      const nextPhoto = await prisma.photo.findFirst({ where: { maisonId: photo.maisonId }, orderBy: { createdAt: 'asc' } });
      if (nextPhoto) await prisma.photo.update({ where: { id: nextPhoto.id }, data: { isMain: true } });
    }
...
writeFile(path.join(backendBase, 'src', 'routes', 'auth.js'), `'use strict';
...
writeFile(path.join(backendBase, 'src', 'routes', 'maisons.js'), `'use strict';
...
writeFile(path.join(backendBase, 'src', 'routes', 'upload.js'), `'use strict';
...
writeFile(path.join(backendBase, 'src', 'server.js'), `'use strict';
...
console.log('\n✅ All done! Backend and frontend structure created successfully.');
